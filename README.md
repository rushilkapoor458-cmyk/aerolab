...
## Also in this repository: [Aurelia](website/README.md)

[`website/`](website/) holds a third, equally unrelated project — **[Aurelia](website/README.md)**,
a marketing and ordering site for a fictional restaurant. Static HTML, CSS and
...
Opening `website/index.html` in a browser works just as well. Every image is
generated by a script in `website/tools/` rather than sourced from stock
photography, and the checkout is local-state UI only — no payment is processed
and nothing leaves the browser. Its own README covers the content files to edit
and the optional production build. As with `atc/`, nothing below applies to it.
